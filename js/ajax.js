...
$(function(){
	$('#submit_reg').click(function(){
		...
		if ((name == "") || (email == "") || (password == "") || (con_password == "")) {
			...
			$('#reg_error').text("Mohon isi semua bagian!");
		}
		else if (!mail_regex.test(email)) {

			$('#reg_error').text('Masukkan Email yang sesuai!');
		}
		else if (!name_regex.test(name)) {

			$('#reg_error').text('Masukkan nama yang sesuai!');
		}
		else if (password != con_password) {

			$('#reg_error').text("Password tidak sama!");
		} else {

			$.ajax({
	            url :'/GoPrekin/backends/register.php',
	            type:'POST',
	            data :{
	            'name':name,
	            'email':email,
	...
	$('#login_btn').click(function(){
		...
		if ((email == "") || (password == "")) {
			...
			$.ajax({
	            url :'/GoPrekin/backends/login.php',
	            type:'POST',
	            data :{
	            'name':name,
	            'email':email,
